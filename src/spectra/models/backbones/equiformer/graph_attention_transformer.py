...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        max_atom_type=_MAX_ATOM_TYPE, class_map=None, node_atom_is_index=False):
        super().__init__()
        ...
        self.node_atom_is_index = bool(node_atom_is_index)
        self.class_map = None
        if class_map is None and self.max_atom_type == 5:
            class_map = [1, 6, 7, 8, 9]
        if class_map is not None:
            self.class_map = [int(x) for x in class_map]
            max_z = max(self.class_map)
            z_to_idx = torch.full((max_z + 1,), -1, dtype=torch.long)
            for i, z in enumerate(self.class_map):
                z_to_idx[z] = i
            self.register_buffer("_z_to_idx", z_to_idx)
        else:
            self.register_buffer("_z_to_idx", None)

        self.atom_embed = NodeEmbeddingNetwork(self.irreps_node_embedding, self.max_atom_type)
        self.basis_type = basis_type
        if self.basis_type == 'gaussian':
            self.rbf = GaussianRadialBasisLayer(self.number_of_basis, cutoff=self.max_radius)
        elif self.basis_type == 'bessel':
            if RadialBasis is None:
                raise ImportError("RadialBasis requires ocpmodels; install or use basis_type='gaussian'.")
    ...
    def forward(self, f_in, pos, batch, node_atom, **kwargs) -> torch.Tensor:
        if _radius_graph is None:
            raise ImportError("radius_graph not available (torch_cluster/torch_geometric missing).")

        edge_src, edge_dst = _radius_graph(pos, r=self.max_radius, batch=batch,
        ...
        if self.node_atom_is_index:
            if node_atom.max() >= self.max_atom_type or node_atom.min() < 0:
                raise ValueError("node_atom index out of range for max_atom_type.")
        else:
            if self._z_to_idx is None:
                raise ValueError("class_map is required when node_atom_is_index=False.")
            if node_atom.max() >= self._z_to_idx.numel():
                raise ValueError("node_atom contains atomic number outside class_map range.")
            node_atom = self._z_to_idx[node_atom]
            if (node_atom < 0).any():
                raise ValueError("node_atom contains atomic number not in class_map.")
