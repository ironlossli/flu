...
# --- compat shim for fairchem-core v1.x (no BaseModel provided) ---
try:
    # 如果未来换到有 BaseModel 的版本，这里会直接用官方的
    from fairchem.core.models.base import BaseModel  # type: ignore
except ImportError:
    from fairchem.core.models.base import GraphModelMixin
    import torch.nn as nn

    class BaseModel(nn.Module, GraphModelMixin):  # 兼容旧代码的基类
        def __init__(self, *args, **kwargs):
            nn.Module.__init__(self)
            # GraphModelMixin 里会读取这个属性；默认设为 False
            if not hasattr(self, "use_pbc_single"):
                self.use_pbc_single = False

        # 旧版项目里很多地方把 generate_graph 当成 6 元组来解包
        # 这里把 v1 的 GraphData 转成旧式返回，避免你改大量代码
        def generate_graph(self, *args, **kwargs):
            g = GraphModelMixin.generate_graph(self, *args, **kwargs)
            return (
                g.edge_index,
                g.edge_distance,
                g.edge_distance_vec,
